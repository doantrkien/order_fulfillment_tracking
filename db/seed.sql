...
BEGIN
    ...
    FOR i IN 1..1000000 LOOP
        ...
        VALUES 
            (new_order_id, 'PENDING', NOW() - '5 hours'::interval, NOW() - '5 hours'::interval, 'system'),
            (new_order_id, 'DELIVERING', NOW() - '2 hours'::interval, NOW() - '2 hours'::interval, 'system');
            
        -- Insert final event for DELIVERED or FAILED status
        IF selected_status = 'DELIVERED' THEN
            INSERT INTO order_events (order_id, new_status, event_at, created_at, updated_by)
            VALUES (new_order_id, 'DELIVERED', NOW(), NOW(), 'system');
        ELSIF selected_status = 'FAILED' THEN
            INSERT INTO order_events (order_id, new_status, event_at, created_at, updated_by)
            VALUES (new_order_id, 'FAILED', NOW(), NOW(), 'system');
        END IF;
    END LOOP;
