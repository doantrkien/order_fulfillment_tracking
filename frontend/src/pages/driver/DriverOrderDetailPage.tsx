...
// ── Icons ─────────────────────────────────────────────────────────────────────
const IconPin: React.FC<{ isLight?: boolean }> = ({ isLight }) => {
  const color = isLight ? "#A07814" : "#C9A84C";
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      <path d="M8 1.5C5.5 1.5 3.5 3.5 3.5 6c0 3.5 4.5 8.5 4.5 8.5S12.5 9.5 12.5 6c0-2.5-2-4.5-4.5-4.5z" stroke={color} strokeWidth="1.1" fill={isLight ? "rgba(160,120,20,0.08)" : "rgba(201,168,76,0.08)"} opacity="0.7" />
      <circle cx="8" cy="6" r="1.5" stroke={color} strokeWidth="1.1" fill="none" opacity="0.7" />
    </svg>
  );
};
const IconPhone: React.FC<{ isLight?: boolean }> = ({ isLight }) => {
  const color = isLight ? "#A07814" : "#C9A84C";
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      <path d="M3 2h3l1.5 3.5-2 1.5A9 9 0 0 0 9 11l1.5-2L14 10.5V14c-6 0-11-5-11-12z" stroke={color} strokeWidth="1.1" strokeLinejoin="round" fill="none" opacity="0.6" />
    </svg>
  );
};
const IconNote: React.FC<{ isLight?: boolean }> = ({ isLight }) => {
  const color = isLight ? "#A07814" : "#C9A84C";
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      <rect x="2" y="2" width="12" height="12" rx="1.5" stroke={color} strokeWidth="1.1" fill="none" opacity="0.6" />
      <path d="M5 6h6M5 9h4" stroke={color} strokeWidth="1.1" strokeLinecap="round" opacity="0.6" />
    </svg>
  );
};
      ...
      <div className="greek-page" style={{ maxWidth: 640, margin: '0 auto' }}>
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            {/* Customer */}
            <div>
              <p className="greek-label" style={FL}>Customer</p>
              <p className="greek-text" style={{ ...FV, fontWeight: 500 }}>{order.username}</p>
            </div>

            {/* Phone — highlighted gold, tappable on mobile */}
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                <IconPhone isLight={isLight} />
                <p className="greek-label" style={{ ...FL, marginBottom: 0 }}>Phone</p>
              </div>
              <a
                href={`tel:${order.user_phone}`}
                style={{ fontFamily: "'Inter', sans-serif", fontSize: 16, fontWeight: 600, color: isLight ? '#C9920A' : '#F0C040', letterSpacing: '0.03em', textDecoration: 'none' }}
              >
                {order.user_phone}
              </a>
            </div>
            ...
            {/* Address */}
            <div>
              <p className="greek-label" style={FL}>Shipping Address</p>
              <p className="greek-text" style={{ ...FV, lineHeight: 1.6 }}>{order.shipping_address}</p>
            </div>
            ...
            {order.driver_note && (
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                  <IconNote isLight={isLight} />
                  <p className="greek-label" style={{ ...FL, marginBottom: 0 }}>Your Note</p>
                </div>
                <div style={{ padding: '10px 14px', background: isLight ? 'rgba(160,120,20,0.06)' : 'rgba(201,168,76,0.06)', borderLeft: `2px solid ${isLight ? 'rgba(160,120,20,0.40)' : 'rgba(201,168,76,0.40)'}`, borderRadius: '0 2px 2px 0' }}>
                  <p className="greek-text" style={{ ...FV, fontStyle: 'italic', margin: 0 }}>
                    {order.driver_note}
                  </p>
                </div>
              </div>
